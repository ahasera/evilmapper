...
sleep 1
echo
# Check if simplehttpserver is running and kill it if it is 
if pgrep -f "python3 -m http.server" > /dev/null; then
    echo "Stopping running simplehttpserver..."
...
perform_scan() {
    ...
    nmap $pn_option $scan_options $target -oX $xml_output
...
if [[ $skip_discovery == [yY] || $skip_discovery == [yY][eE][sS] ]]; then
    ...
    echo "Performing host discovery phase..."
fi
...
select category in "${categories[@]}"; do
    case $category in
        ...
        "Version Detection")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Service Detection") perform_scan "Service Detection" "-sV"; break 2;;
                    "OS Detection") perform_scan "OS Detection" "-O"; break 2;;
                    "Attempt OS Guessing") perform_scan "OS Guessing" "-O --osscan-guess"; break 2;;
                    "Increasing Version Detection") perform_scan "Version Intensity" "-sV --version-intensity 5"; break 2;;
                    "Troubleshoot Version Scans") perform_scan "Version Trace" "-sV --version-trace"; break 2;;
                    "Aggressive Detection Mode") perform_scan "Aggressive Detection" "-A"; break 2;;
                    "Verbose Mode") perform_scan "Verbose OS Detection" "-O -v"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "Network and Port Scanning")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "TCP SYN Ping Scan") perform_scan "TCP_SYN_Ping_Scan" "-sn -PS $target"; break 2;;
                    "Scanning Multiple Ports") perform_scan "Multiple_Ports_Scan" "-sn -PS80,100-1000 $target"; break 2;;
                    "TCP ACK Ping Scan") perform_scan "TCP_ACK_Ping_Scan" "-sn -PA $target"; break 2;;
                    "UDP Ping Scan") perform_scan "UDP_Ping_Scan" "-sn -PU $target"; break 2;;
                    "ICMP Ping Scan") perform_scan "ICMP_Ping_Scan" "-sn -PE $target"; break 2;;
                    "SCTP INIT Ping Scan") perform_scan "SCTP_INIT_Ping_Scan" "-sn -PY $target"; break 2;;
                    "IP Protocol Ping Scan (Tracing)") perform_scan "IP_Protocol_Ping_Scan" "-sn -PO --packet-trace $target"; break 2;;
                    "Scan Random Number of Hosts")
                        ...
                        break 2;;
                    "Broadcast Ping Scan") perform_scan "Broadcast_Ping_Scan" "--script broadcast-ping --packet-trace $target"; break 2;;
                    "Xmas Scan") perform_scan "Xmas_Scan" "-sX $target"; break 2;;
                    "UDP Scan (Verbose)") perform_scan "UDP_Verbose_Scan" "-sU -v $target"; break 2;;
                    "Scan a Firewall") perform_scan "Firewall_Scan" "-f $target"; break 2;;
                    "Cloak Scan with Decoys")
                        read -p "Enter decoy IPs (comma-separated, no spaces): " decoys
                        perform_scan "Decoy_Scan" "-D $decoys $target"
                        break 2;;
                    "Spoof Source IP Address")
                        read -p "Enter IP address to spoof: " spoof_ip
                        perform_scan "Spoof_IP_Scan" "-S $spoof_ip $target"
                        break 2;;
                    "Spoof MAC Address")
                        read -p "Enter MAC address to spoof (or 0 for random): " spoof_mac
                        perform_scan "Spoof_MAC_Scan" "--spoof-mac $spoof_mac $target"
                        break 2;;
                    "Scan Using a Random MAC Address") perform_scan "Random_MAC_Scan" "--spoof-mac 0 $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Rate Limiting")
                        ...
                        perform_scan "Rate_Limiting" "--scan-delay $rate_limit $target"
                        break 2;;
                    "Adjust Delay Between Probes")
                        read -p "Enter initial delay time (e.g., 0.1s): " init_delay
                        read -p "Enter maximum delay time (e.g., 1s): " max_delay
                        perform_scan "Adjust_Delay" "--scan-delay $init_delay --max-scan-delay $max_delay $target"
                        break 2;;
                    # ... [Other timing options] ...
                    "Host Timeouts – Give up on Hosts")
                        read -p "Enter host timeout (e.g., 5m): " host_timeout
                        perform_scan "Host_Timeouts" "-sV -A -p- --host-timeout $host_timeout $target"
                        break 2;;
                    "Paranoid Timing Template") perform_scan "Paranoid_Timing" "-T0 $target"; break 2;;
                    "Sneaky – ID Evasion (T0)") perform_scan "Sneaky_ID_Evasion" "-T1 $target"; break 2;;
                    "Polite – Slower Than Normal Scan") perform_scan "Polite_Slower" "-T2 $target"; break 2;;
                    "Normal – Default Speed") perform_scan "Normal_Default" "-T3 $target"; break 2;;
                    "Aggressive – Recommended Mode") perform_scan "Aggressive_Recommended" "-T4 -n -Pn -p- $target"; break 2;;
                    "Insane – Very Fast Networks") perform_scan "Insane_Very_Fast" "-T5 $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "NSE Scripts")
           ...
           echo
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Safe Category – Default") perform_scan "Safe_Category_Default" "-sC $target"; break 2;;
                    "Execute Scripts by Name") perform_scan "Scripts_by_Name" "--script default,safe $target"; break 2;;
                    "Select Script by Category")
                        read -p "Enter script category (e.g., exploit): " script_category
                        perform_scan "Script_by_Category" "--script $script_category $target"
                        break 2;;
                    "Execute NSE Script File")
                        read -p "Enter path to NSE script file: " script_path
                        perform_scan "NSE_Script_File" "--script $script_path $target"
                        break 2;;
                    "Exclude a Specific Category")
                        read -p "Enter category to exclude (e.g., exploit): " exclude_category
                        perform_scan "Exclude_Category" "-sV --script \"not $exclude_category\" $target"
                        break 2;;
                    "Include Two Different Categories")
                        read -p "Enter two categories to include (e.g., broadcast,discovery): " include_categories
                        perform_scan "Include_Categories" "--script \"$include_categories\" $target"
                        break 2;;
                    "Combining Wildcards") perform_scan "Combining_Wildcards" "--script \"http-*\" $target"; break 2;;
                    "Set Arguments")
                        read -p "Enter script and arguments (e.g., http-title --script-args http.useragent='Mozilla'): " script_args
                        perform_scan "Set_Arguments" "-sV --script $script_args $target"
                        break 2;;
                    "Load Arguments from a File")
                        read -p "Enter script and args file path (e.g., discovery nmap-args.txt): " script_args_file
                        perform_scan "Load_Args_From_File" "--script \"$script_args_file\" $target"
                        break 2;;
                    "Back") break;;
                esac
            done
        ...
        "Web Server Scanning")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "List Supported HTTP Methods") perform_scan "List_HTTP_Methods" "-p80,443 --script http-methods --script-args httpmethods.test-all=true $target"; break 2;;
                    "Discover Interesting Paths/Folders") perform_scan "Discover_Paths" "--script http-enum -sV $target"; break 2;;
                    "Brute-force HTTP Basic Auth") perform_scan "HTTP_Basic_Auth_Brute" "-p80 --script http-brute $target"; break 2;;
                    "Provide Own User/Password List")
                        read -p "Enter path to username list: " user_list
                        read -p "Enter path to password list: " pass_list
                        perform_scan "User_Pass_Brute" "-sV --script http-brute --script-args userdb=$user_list,passdb=$pass_list $target"
                        break 2;;
                    "Brute-force Common Web Platforms") perform_scan "Web_Platforms_Brute" "-sV --script http-wordpress-brute $target"; break 2;;
                    "Detect Web Application Firewall") perform_scan "Detect_WAF" "-sV --script http-waf-detect,http-waf-fingerprint $target"; break 2;;
                    "Detect XST Vulnerabilities") perform_scan "Detect_XST" "-sV --script http-methods,http-trace --script-args http-methods.retest $target"; break 2;;
                    "Detect XSS Vulnerabilities") perform_scan "Detect_XSS" "-sV --script http-unsafe-output-escaping $target"; break 2;;
                    "Detect SQL Injection Vulnerabilities") perform_scan "Detect_SQL_Injection" "-sV --script http-sql-injection $target"; break 2;;
                    "Find Default Credentials") perform_scan "Find_Default_Credentials" "-sV --script http-default-accounts $target"; break 2;;
                    "Find Exposed Git Repos") perform_scan "Find_Git_Repos" "-sV --script http-git $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "Mail Server Scanning")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Brute-force SMTP") perform_scan "Brute_force_SMTP" "-p25 --script smtp-brute $target"; break 2;;
                    "Brute-force IMAP") perform_scan "Brute_force_IMAP" "-p143 --script imap-brute $target"; break 2;;
                    "Brute-force POP3") perform_scan "Brute_force_POP3" "-p110 --script pop3-brute $target"; break 2;;
                    "Enumerate SMTP Users") perform_scan "Enumerate_SMTP_Users" "-p 25 --script smtp-enum-users $target"; break 2;;
                    "SMTP on Alternate Ports") perform_scan "SMTP_Alternate_Ports" "-sV --script smtp-strangeport $target"; break 2;;
                    "Discover Open Relays") perform_scan "Discover_Open_Relays" "-sV --script smtp-open-relay -v $target"; break 2;;
                    "Find Available SMTP Commands") perform_scan "Available_SMTP_Commands" "-p 25 --script smtp-commands $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "Database Scanning")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Identify MS SQL Servers") perform_scan "Identify_MS_SQL" "-p1433 --script ms-sql-info $target"; break 2;;
                    "Brute-force MS SQL Passwords") perform_scan "Brute_force_MS_SQL" "-p1433 --script ms-sql-brute $target"; break 2;;
                    "Dump MS SQL Password Hashes") perform_scan "Dump_MS_SQL_Hashes" "-p1433 --script ms-sql-empty-password,ms-sql-dump-hashes $target"; break 2;;
                    "List MySQL Databases")
                        read -p "Enter MySQL username: " mysql_user
                        read -p "Enter MySQL password: " mysql_pass
                        perform_scan "List_MySQL_DB" "-p3306 --script mysql-databases --script-args mysqluser=$mysql_user,mysqlpass=$mysql_pass $target"
                        break 2;;
                    "Brute-force MySQL Passwords") perform_scan "Brute_force_MySQL" "-p3306 --script mysql-brute $target"; break 2;;
                    "MySQL Root/Anonymous Empty Passwords") perform_scan "MySQL_Empty_Passwords" "-p3306 --script mysql-empty-password $target"; break 2;;
                    "Brute-force Oracle SIDs") perform_scan "Brute_force_Oracle_SID" "-sV --script oracle-sid-brute $target"; break 2;;
                    "Identify MongoDB Servers") perform_scan "Identify_MongoDB" "-p27017 --script mongodb-info $target"; break 2;;
                    "List CouchDB Databases") perform_scan "List_CouchDB_DB" "-p5984 --script couchdb-databases $target"; break 2;;
                    "Identify Cassandra Databases") perform_scan "Identify_Cassandra" "-p9160 --script cassandra-brute $target"; break 2;;
                    "Brute-force Redis Passwords") perform_scan "Brute_force_Redis" "-p6379 --script redis-brute $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "Top Commands")
            ...
            select opt in "${options[@]}"; do
                case $opt in
                    "Comprehensive Scan") perform_scan "Comprehensive_Scan" "-A -T4 -v -sS -sV -sC -O --script=all --script-args=safe=1 --open --reason $target"; break 2;;
                    "Back") break;;
                esac
            done
            ;;
        ...
        "Custom Scan")
            ...
            perform_scan "Custom" "$custom_opts"
            break
            ;;
    ...
    start_python_server() {
        ...
        if test -f "scans/html/server/server.py"; then
            echo ""
        else
            echo "Server.py not found moving it to scans folder"
            mv server.py scans/html/server/
        fi
    ...
    # Trap SIGINT signal to stop Python server when user quits
